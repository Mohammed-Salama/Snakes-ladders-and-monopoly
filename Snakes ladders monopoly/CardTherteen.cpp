...
bool CardTherteen::isFirst = true;
Player* CardTherteen::owner = NULL;
int CardTherteen::cardPrice = 0;
int CardTherteen::fees = 0;
//constructor
CardTherteen::CardTherteen(const CellPosition& pos) : Card(pos)
{
	cardNumber = 13;
	bool added= true;
...
CardTherteen::CardTherteen()
{

}

bool CardTherteen::getadded()
{
	return added;
}
//reading prameters from the user if it is the first time
void CardTherteen::ReadCardParameters(Grid* pGrid)
{
	if (isFirst)
	{
...
void CardTherteen::Apply(Grid* pGrid, Player* pPlayer)
{
	Input* pIn = pGrid->GetInput();
	Output* pOut = pGrid->GetOutput();
	int wallet = pPlayer->GetWallet();
	string choice;
	//Offerring to buy all cells that has card 13
	if (owner == NULL)
	{
		...
		if (choice == "y")
		{
			...
			else
			{
				...
				pOut->PrintMessage("congratulations, you now own all  the cells that has card 13 ,click to continue... ");
			}
		}
		else
...
void CardTherteen::SetBool()
{
	isFirst = true;
}
Card* CardTherteen::GetCopy(CellPosition pos)
{
	return new  CardTherteen (pos);
...
void CardTherteen::Save(ofstream& OutFile, ObjectType Type)  // Saves the GameObject parameters to the file
{
	if (Type == CARDTHIRTEEN)
	{
...
void CardTherteen::Load(ifstream& Infile, GameObject* p, Grid* pGrid) 	// Loads and Reads the GameObject parameters from the file
{
