...
void CardTwelve::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	string choice;
	//Offerring to buy all cells that has card 12
	if (owner == NULL)
	{
		...
		if (choice == "y")
		{
			...
			else
			{
				...
				pOut->PrintMessage("congratulations, you now own all  the cells that has card 12 ,click to continue... ");
			}
		}
		else
...
void CardTwelve::Save(ofstream& OutFile, ObjectType Type)  // Saves the GameObject parameters to the file
{
	if (Type == CARDTWELVE)
	{
...
void CardTwelve::Load(ifstream& Infile, GameObject* p, Grid* pGrid) 	// Loads and Reads the GameObject parameters from the file
{
	int cellnum;
	cardNumber = 12;
