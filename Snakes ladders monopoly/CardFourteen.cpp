...
bool CardFourteen::isFirst = true;
...
void CardFourteen::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	string choice;
	//Offerring to buy all cells that has card 14
	if (owner == NULL)
	{
		...
		if (choice == "y")
		{
			...
			else
			{
				...
				pOut->PrintMessage("congratulations, you now own all  the cells that has card 14 ,click to continue... ");
			}
		}
		else
...
void CardFourteen::SetBool()
{
	isFirst = true;
}
Card* CardFourteen::GetCopy(CellPosition pos)
{
	return new  CardFourteen (pos);
}
CardFourteen::~CardFourteen()
{

}
void CardFourteen::Save(ofstream& OutFile, ObjectType Type)  // Saves the GameObject parameters to the file
{
	if (Type == CARDFOURTEEN)
	{
...
void CardFourteen::Load(ifstream& Infile, GameObject* p, Grid* pGrid) 	// Loads and Reads the GameObject parameters from the file
{
	int cellnum;
	cardNumber = 14;
