...
void CardEleven::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	string choice;
	//Offerring to buy all cells that has card 11
	if (owner == NULL)
	{
		...
		if (choice == "y")
		{
			...
			else
			{
				...
				pOut->PrintMessage("congratulations, you now own all  the cells that has card 11 ,click to continue... ");
			}
		}
		else
...
void CardEleven::Save(ofstream& OutFile, ObjectType Type)  // Saves the GameObject parameters to the file
{
	if (Type == CARDELEVEN)
	{
...
void CardEleven::Load(ifstream& Infile, GameObject* p, Grid* pGrid) 	// Loads and Reads the GameObject parameters from the file
{
	int cellnum;
	cardNumber = 11;
